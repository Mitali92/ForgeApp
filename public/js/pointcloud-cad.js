...
export default class PointCloudExtension extends Autodesk.Viewing.Extension {
    ...
    _renderCloud(){
          ...
          this.pointsParticle = new THREE.PointCloud(
            this._generatePointCloudGeometry(0),
          ...
          this.pointsHuman = new THREE.PointCloud(
            this._generatePointCloudGeometry(2),
            this._generateShaderMaterial()
            );
     ...
     */
    _generatePointCloudGeometry(iconselection) {
        ...
        for (var i = 0; i < simulation.Frames.length; i++){
            ...
            for (var j = 0; j < frame.StateMessages.length; j++) {
                ...
                let u = m.X / simulation.MaxX - xaxisOffset;//0;
                let v = m.Y / simulation.MaxY - yaxisOffset;//0;
    ...
    _generateShaderMaterial() {
        ...
        void main() {
           gl_FragColor = vec4(vColor,uVu.y) ;//* texture2D( sprite, gl_PointCoord );
           gl_FragColor = gl_FragColor * texture2D(sprite, vec2((gl_PointCoord.x+uVu.x*1.0)/4.0, 1.0-gl_PointCoord.y));
        ...
        return new THREE.ShaderMaterial( {
            uniforms: {
                ...
                sprite: { type: 't', value: THREE.ImageUtils.loadTexture("../img/4ImgList.png") },
            },
