...
export default Core.Templatable("Widget.Loader", class Loader extends Templated { 
	...
	UpdateButton() {
		this.Elem("parse").disabled = dropzones.files.length == 0; //this.Widget("dropzone").files.length == 0;
	}
	...
	Load() {
		var spinners = document.getElementsByClassName("loading-hide");
            if (spinners.length == 0) return;
            var spinner = spinners[0];
            spinner.classList.remove("loading-hide");
            spinner.classList.add('loading-show');
			
		



		this.Files = dropzone.files;

		//this.Parse(this.Files.convert)
	...
	OnParser_Finished(responses) {	
		...
		window.configuration = output.configuration;
		window.style = output.style;

		var spinners = document.getElementsByClassName("loading-show");
         if (spinners.length == 0) return;
         var spinner = spinners[0];
